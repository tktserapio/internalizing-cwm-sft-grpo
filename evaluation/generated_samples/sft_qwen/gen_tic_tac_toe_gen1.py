...
def check_win(state: State) -> bool:
    """
    Checks if there is a winning line of four marks.
    """
    ...
    # Check diagonals
    for i in range(3):
        if "".join(state["board"][j][j-i] for j in range(i+1)) == "xxxx" or \
           "".join(state["board"][j][i+j] for j in range(6-i)) == "xxxx":
            return True
...
def get_current_player(state: State) -> int:
    ...
    """
    return state["current_player"] if not state["game_over"] else -4
        ...
        for row in range(6):
            for col in range(6):
                if state["board"][row][col] == player:
                    observation[f"{row},{col}"] = 1.0
                else:
                    observation[f"{row},{col}"] = 0.0
