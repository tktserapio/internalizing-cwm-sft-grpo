...
def check_winner(state: State) -> None:
    ...
    If a winner is found, sets the 'winner' field in the state dictionary.
    """
    ...
    for diag in [(0, 0), (0, 5), (5, 0), (5, 5)]:
        if len(set(state["board"][r][c] for r, c in zip(diag, diag))) == 1 and state["board"][diag[0]][diag[1]] != ".":
            state["winner"] = state["board"][diag[0]][diag[1]]
            return
...
def get_rewards(state: State) -> List[float]:
    ...
    if state["winner"] is not None:
        return [1.0, 0.0] if state["winner"] == "x" else [0.0, 1.0]
    elif state["is_draw"]:
        return [0.5, 0.5]
    else:
        return [0.0, 0.0]
...
def get_observations(state: State) -> List[PlayerObservation]:
    ...
    return [{"board": state["board"], "current_player": state["current_player"]}, {"board": state["board"], "current_player": state["current_player"]}]