...
def apply_action(state: State, action: Action) -> State:
    ...
    if action == "Draw stock":
        ...
        # Additional logic for scoring and ending the round
    elif action == "Pass":
        new_state['current_player'] = 1 - current_player
    elif action == "Action: Done":
        # End of knock sequence
        pass
...
def get_rewards(state: State) -> List[float]:
    ...
    if not state['round_over']:
        return [0.0, 0.0]
    # Calculate rewards based on the round outcome
    return state['scores']
...
def resample_history(obs_action_history: List[Tuple[PlayerObservation, Action | None]], player_id: int) -> List[Action]:
    ...
    """
    # This function would need to reconstruct the sequence of actions leading to the current state
    # For simplicity, we'll return an empty list here
    return []

# Helper functions can be added here for specific game logic, such as calculating deadwood, checking for melds, etc.