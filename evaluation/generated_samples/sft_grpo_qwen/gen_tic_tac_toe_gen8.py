...
def check_winner(state: State):
    ...
    if len(set([state["board"][i][i] for i in range(6)])) == 1 and state["board"][0][0] != ".":
        state["winner"] = "x" if state["board"][0][0] == "x" else "o"
        return
    
    if len(set([state["board"][i][5-i] for i in range(6)])) == 1 and state["board"][0][5] != ".":
...
def get_observations(state: State) -> List[PlayerObservation]:
    ...
    player_0_obs = {"board": state["board"], "current_player": get_current_player(state)}
    player_1_obs = {"board": state["board"], "current_player": get_current_player(state)}
