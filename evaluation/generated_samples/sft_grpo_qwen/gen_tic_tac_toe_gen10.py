...
def check_winner(state: State):
    ...
    for i in range(3):
        if len(set([state["board"][i][i], state["board"][i+1][i+1], state["board"][i+2][i+2], state["board"][i+3][i+3]])) == 1 and state["board"][i][i] != ".":
            state["winner"] = state["board"][i][i]
            return
        if len(set([state["board"][i][5-i], state["board"][i+1][4-i], state["board"][i+2][3-i], state["board"][i+3][2-i]])) == 1 and state["board"][i][5-i] != ".":
            state["winner"] = state["board"][i][5-i]
            return
    
    # No winner found
    if state["winner"] is None:
        state["winner"] = "ongoing"
